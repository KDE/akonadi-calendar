...
SuspendDialog::SuspendDialog(const KSharedConfig::Ptr &config, const QString &title, const QString &text, QWidget *parent)
    ...
    , m_config(config)
{
    KConfigGroup configGroup(config, "Suspend");
    const int suspendVal = configGroup.readEntry("DefaultSuspendValue", defSuspendVal);
    const SuspendUnit suspendUnit = configGroup.readEntry("DefaultSuspendUnit", defSuspendUnit);
    ...
    connect(suspendButton, &QPushButton::clicked, this, [this, suspendSpin, suspendUnitCombo]() {
        KConfigGroup configGroup(m_config, "Suspend");
        configGroup.writeEntry("DefaultSuspendValue", suspendSpin->value());
        configGroup.writeEntry("DefaultSuspendUnit", static_cast<SuspendUnit>(suspendUnitCombo->currentIndex()));
