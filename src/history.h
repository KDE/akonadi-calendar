...
/*!
   \brief History class for implementing undo/redo of calendar operations
...
public:
     ...
     * undone calling undo().
     *
     * \a item the item that was created. Must be valid and have a Incidence::Ptr payload
     * \a description text that can be used in the undo/redo menu item to describe the operation
     *        If empty, a default one will be provided.
     * \a atomicOperationId if not 0, specifies which group of changes this change belongs to.
     ...
     * undo().
     *
     * \a oldItem item containing the payload before the change. Must be valid
     *        and contain an Incidence::Ptr payload.
     * \a newItem item containing the new payload. Must be valid and contain
     ...
     * undone calling undo().
     *
     * \a item The item to delete. Must be valid, doesn't need to contain a
     *        payload.
     * \a description text that can be used in the undo/redo menu item to describe the operation
     *        If empty, a default one will be provided.
     * \a atomicOperationId if not 0, specifies which group of changes this change belongs to.
     ...
     *
     * \a items The list of items to delete. All items must be valid.
     * \a description text that can be used in the undo/redo menu item to describe the operation
     *        If empty, a default one will be provided.
     * \a atomicOperationId If != 0, specifies which group of changes thischange belongs to.
     ...
     *
     * \a parent will be passed to dialogs created by IncidenceChanger,
...
public Q_SLOTS:
     ...
     *
     * Returns true if the stacks were cleared, false if there was a job running
     */
    bool clear();
